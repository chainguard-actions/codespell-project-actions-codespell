...
# Build argument list using positional parameters to avoid word-splitting
# and shell injection from unquoted variable expansions.
set --

echo "Builtin dictionaries '${INPUT_BUILTIN}'"
if [ "x${INPUT_BUILTIN}" != "x" ]; then
    set -- "$@" --builtin "${INPUT_BUILTIN}"
fi
